...
class FRF_Admin_Settings {
    ...
    private static function save_settings() {
        ...
        // 1. Start with existing settings so we don't wipe out other tabs
        $new_settings = $settings->get_all();
        
        // 2. Identify the current tab to know which fields to update
        $tab = isset($_GET['tab']) ? sanitize_text_field($_GET['tab']) : 'general';
        
        // 3. Update fields based on the active tab
        if ($tab === 'general') {
        ...
        if ($tab === 'invoices') {
            ...
            // Checkboxes only send data if checked, so isset() is correct here inside the conditional block
        ...
        if (is_wp_error($validation)) {
            ...
            if ($settings->update($new_settings)) {
                ...
                // "Error saving" often appears if data didn't change, which is normal in WP options
                add_settings_error('frf_messages', 'frf_message', __('Settings saved (or no changes made)', 'fatture-rf'), 'success');
            }
        }
    }
